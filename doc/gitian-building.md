...
*Setup instructions for a gitian build of PIPOCOIN using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PIPOCOIN
...
- [Building PIPOCOIN](#building-pipocoin)
...
Clone the git repositories for pipocoin and gitian and then checkout the pipocoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/pipocoin-crypto/pipocoin.git
cd pipocoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pipocoin repository
...
To build PIPOCOIN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the pipocoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pipocoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/pipocoin-crypto/pipocoin
...
For example:
```bash
URL=https://github.com/crowning-/pipocoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pipocoin=${COMMIT} --url pipocoin=${URL} ../pipocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pipocoin=${COMMIT} --url pipocoin=${URL} ../pipocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pipocoin=${COMMIT} --url pipocoin=${URL} ../pipocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pipocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pipocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pipocoin-build.assert
```
...
[pipocoin/gitian.sigs](https://github.com/pipocoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
