...
Please report bugs using the issue tracker at github: <https://github.com/pipo-project/PiPo-coin/issues>
...
### Deterministic zPIPO Seed Keys

zPIPO is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zPIPO outside of the wallet that the zPIPO had been minted on. zPIPO can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zPIPO minting

zPIPO minting now only requires 1 further mint (down from 2) to mature. zPIPO mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zPIPO Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zPIPO denomination has been spent or not.



PIPO/zPIPO Staking and Masternode Rewards
--------------

### PIPO, zPIPO and Masternode Payment Schedule

To encourage the use of zPIPO and increase the PIPOCOIN zerocoin anonymity set, the PIPOCOIN payment schedule has been changed to the following:

If a user staking zPIPO wins the reward for their block, the following zPIPO reward will be: 
- 3 zPIPO (3 x 1 denominations) rewarded to the staker, 2 PIPO rewarded to the masternode owner and 1 PIPO available for the budget. This is a total block reward of 6 PIPO, up from 5.

If a user staking PIPO wins the reward, the following amounts will be rewarded: 
- 2 PIPO to the PIPO staker, 3 PIPO to the Masternode owner and 1 PIPO available for the budget. This is a total block reward of 6 PIPO, up from 5.


### Return change to sender when minting zPIPO

Previously, zPIPO minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most PIPO and return the change from a zPIPO mint to that address.
...
The visual layout of the PIPOCOIN Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zPIPO minting feature. This is enabled by default and the enablezeromint=0 setting in the pipocoin.conf file will overwrite the GUI option.
-	The percentage of autominted zPIPO can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular PIPO.
...
### Summary
 ...
 The PIPOCOIN wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the pipocoin.conf file, found in the PIPOCOIN data directory.
- backuppath = <directory / full path>
- zpipobackuppath = <directory / full path>
...
* If zPIPO backup, auto generated name is `wallet-autozpipobackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
PIPOCOIN Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The PIPOCOIN client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the PIPOCOIN support group, which often times sees users say "I didn't spend that zPIPO". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the PIPOCOIN masternodes, the monthly budget available to be utilised has been increased to 42,000 PIPO / month. This PIPO only has the opportunity to be raised once per month (paid to winning proposals) with any unused PIPO not created by the blockchain.

### Budget Finalization Fee

The PIPOCOIN finalization fee for successful proposals has now been reduced, this fee is now 5 PIPO down from 50 PIPO. The total fee outlay for a successful proposal is now a total of 55 PIPO.
...
-	Enforced v1 zPIPO spends to require a security level of 100
-	Updates to zPIPO spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the PIPOCOIN-Qt masternodes tab
-	Updated PIPOCOIN-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and PIPOCOIN's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zPIPO Control dialog in the QT wallet, a user is able to see zPIPO marked as version 1 or 2.

zPoS (zPIPO staking)
--------------

Once a zPIPO has over 200 confirmations it becomes available to stake. Staking zPIPO will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zPIPO. So for example if a 1,000 zPIPO denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zPIPOs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within PIPOCOIN's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zPIPO, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zPIPO that can be recalculated at any time using the seed. Deterministic zPIPO allows for users to backup all of their future zPIPO by simply recording their seed and keeping it in a safe place (similar to backing up a private key for PIPO). The zPIPO seed needs to remain in the wallet in order to spend the zPIPO after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zPIPO data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
...
### Core Features
 - #582 `cb1632520` [Core] zPIPO v2: zPoS dzPIPO ezPIPO external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zPIPO balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zPIPO status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault pipocoin-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zPIPO spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zPIPO. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zPIPO. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/pipocoin-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
