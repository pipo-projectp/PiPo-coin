...
int nAnonymizePipocoinAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "pipocoin" is a composite category enabling all PIPOCOIN-related debug output
            if (ptrCategory->count(string("pipocoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "pipocoin.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty pipocoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override pipocoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "pipocoind.pid"));
