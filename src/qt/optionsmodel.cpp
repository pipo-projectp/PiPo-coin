...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PIPO);
    ...
    if (!settings.contains("nAnonymizePipocoinAmount"))
        settings.setValue("nAnonymizePipocoinAmount", 1000);

    nAnonymizePipocoinAmount = settings.value("nAnonymizePipocoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePipocoinAmount"))
        SoftSetArg("-anonymizepipocoinamount", settings.value("nAnonymizePipocoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pipocoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePipocoinAmount:
            return QVariant(nAnonymizePipocoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizePipocoinAmount:
            nAnonymizePipocoinAmount = value.toInt();
            settings.setValue("nAnonymizePipocoinAmount", nAnonymizePipocoinAmount);
            emit anonymizePipocoinAmountChanged(nAnonymizePipocoinAmount);
            break;
        case CoinControlFeatures:
