...
#include "ui_zpipocontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zpipoTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpipoTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zPIPO seed used to mint this zPIPO is not the same as currently hold in the wallet";
            else
