// Copyright (c) 2018 The PIPOCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zpipo/deterministicmint.h>
#include "zpipotracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzPIPOTracker::CzPIPOTracker(std::string strWalletFile)
{
...
CzPIPOTracker::~CzPIPOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPIPOTracker::Init()
{
...
bool CzPIPOTracker::Archive(CMintMeta& meta)
{
...
bool CzPIPOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPIPOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPIPOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPIPOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPIPOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPIPOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPIPOTracker::GetSerialHashes()
{
...
CAmount CzPIPOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPIPOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPIPOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPIPOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPIPOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPIPOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPIPOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPIPOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPIPOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPIPOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPIPOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPIPOWallet* zPIPOWallet)
{
    bool iszPIPOWalletInitialized = (NULL != zPIPOWallet);
    CMintMeta meta;
    ...
    if (! iszPIPOWalletInitialized)
        zPIPOWallet = new CzPIPOWallet(strWalletFile);
    meta.isSeedCorrect = zPIPOWallet->CheckSeed(dMint);
    if (! iszPIPOWalletInitialized)
        delete zPIPOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPIPOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPIPOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPIPOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPIPOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPIPOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPIPOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPIPOWallet* zPIPOWallet = new CzPIPOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zPIPOWallet);
        }
        delete zPIPOWallet;
        LogPrint("zero", "%s: added %d dzpipo from DB\n", __func__, listDeterministicDB.size());
    }
