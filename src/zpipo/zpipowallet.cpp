...
CzPIPOWallet::CzPIPOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpipo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPIPOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPIPOSeed_deprecated()) {
                    LogPrintf("%s: Updated zPIPO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpipo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpipo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPIPOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPIPOCount(nCountLastUsed);
    else if (!walletdb.ReadZPIPOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPIPOWallet::Lock()
{
    seedMaster = 0;
}

void CzPIPOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPIPOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPIPO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPIPOWallet::LoadMintPoolFromDB()
{
...
void CzPIPOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPIPOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPIPOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpipoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPIPOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPIPO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpipoTracker which also adds to database
    pwalletMain->zpipoTracker->Add(dMint, true);
...
void CzPIPOWallet::SeedToZPIPO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPIPOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPIPOWallet::GenerateDeterministicZPIPO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPIPOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPIPOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPIPOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
