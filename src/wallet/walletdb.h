...
public:
    ...
    bool WriteZPIPOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPIPOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPIPOSeed_deprecated(uint256& seed);
    bool EraseZPIPOSeed();
    bool EraseZPIPOSeed_deprecated();

    bool WriteZPIPOCount(const uint32_t& nCount);
    bool ReadZPIPOCount(uint32_t& nCount);
