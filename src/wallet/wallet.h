...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zpipospends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PIPO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPIPO send
enum ZerocoinSpendStatus {
    ZPIPO_SPEND_OKAY = 0,                            // No error
    ZPIPO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPIPO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPIPO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPIPO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPIPO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPIPO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPIPO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPIPO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPIPO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPIPO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPIPO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPIPO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPIPO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPIPO_SPENT_USED_ZPIPO = 14,                      // Coin has already been spend
    ZPIPO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPIPO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZPIPOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpipoAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzPIPOTracker> zpipoTracker;
    ...
    void setZWallet(CzPIPOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpipoTracker = std::unique_ptr<CzPIPOTracker>(new CzPIPOTracker(strWalletFile));
    }

    CzPIPOWallet* getZWallet() { return zwalletMain; }
    ...
    /** zPIPO reset */
    boost::signals2::signal<void()> NotifyzPIPOReset;
