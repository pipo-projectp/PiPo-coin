...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpipoTracker = unique_ptr<CzPIPOTracker>(new CzPIPOTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZPIPO_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZPIPO_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
