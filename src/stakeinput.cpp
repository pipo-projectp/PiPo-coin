...
// The zPIPO block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPIPO is a hash of the serial
...
{
    //Create an output returning the zPIPO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPIPOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPIPO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPIPO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPIPOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPIPO output", __func__);
...
{
    CzPIPOTracker* zpipoTracker = pwallet->zpipoTracker.get();
    CMintMeta meta;
    if (!zpipoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpipoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PIPO stake is the outpoint
